...
function forkProcess(env) {
    let wk = cluster_1.fork(env);
    wk.on("exit", function () {
...
function start() {
    if (cluster_1.isMaster) {
        ...
        function readConfig() {
            let f = fs_1.readFileSync(startJson).toString();
            return JSON.parse(f);
        }
        ...
        if (configInfo.exec_mode == "cluster_mode" || configInfo.exec_mode == "cluster") {
            let instance = 1;
            if (configInfo.instances != undefined)
                instance = configInfo.instances || os_1.cpus().length;
            if (instance > 1) {
